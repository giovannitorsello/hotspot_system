...
export const useUserStore = defineStore({
  ...
  actions: {
    async fetchUserProfile(username, password){
        const res = await axios.post('http://localhost/admin/login',{username: username, password: password});
        this.user= res.data.user;
        this.fetchUserData();
    },
     async fetchUserData(){
        const res = await axios.post('http://localhost/admin/data/dataReseller',{user:this.user});
