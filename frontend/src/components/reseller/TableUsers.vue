<template>
  <div>
    <v-row>
      <v-col cols="auto"> <v-btn icon="fas fa-plus" @click="editResellerUser" /> </v-col>
    </v-row>
    <v-row>
    ...
    methods: {
        ...
        axios
          ...
          .then((response) => {
            if (response.data.status == 200) {
              utilityArrays.updateElementById(this.hsComponentStore.usersOfSelectedReseller, response.data.user);
      ...
      deleteResellerUser(user) {
        axios.post("/api/user/delete", { user: user }).then(async (response) => {
          if (response.data.status == 200) {
            utilityArrays.deleteElementById(this.hsComponentStore.usersOfSelectedReseller, response.data.user);
