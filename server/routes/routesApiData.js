...
router.post("/api/customer/getCustomerFromPin", async function (req, res) {
    if (! req.body || req.body.pin) {
        res.send({status: "404", msg: "Nessun Pin inserito", customer: {}});
    }
    var customer = await database.getCustomerFromPin(req.body.pin);
    res.send({status: "200", msg: "Customer Found", customer: customer});
})
...
router.post("/api/ticket/save", async (req, res) => {
    ...
    if (foundTicket == null) { // NEW INSERT
